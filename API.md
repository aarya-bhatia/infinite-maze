...
![image](resources/drawio_2.png)

For this second example, we explain how our algorithm will work. Suppose we are in the orange region in the SW child of the world root. Inside that, suppose we are at the red NW region. Let the exit direction be North. The goal is for the target node to be the Red SW node of the NW region of the world root. To get to that we need to follow these steps: `current.parent.parent.north_west.south_west`. So, we are going one level outside but returning to the original 'region' one level up. Then we keep going south_west as long as there is a south_west node. Here, we only go one level deep. Finally, we reach the red SW region. The simpler case is if the exit direction is East and we land within the same subtree only. This idea motivated us to write an algorithm for finding the target node. Below is the pseudo code:

```{}
parent = current.parent

if target node lies in the same parent node:
  region = target node's region in the parent
  return current.parent.get_child(region)
else:
  currentRegion = current.find_region() # Will be 'NW'
  targetNodeParent = current.parent.parent.get_child(currentRegion)
  flipDir = Flip the first letter direction for current region and append the second letter (Ex. NW to SW, NE to SE, SW to NW etc)
  child = targetNodeParent.get_child(flipDir)
  if not child:
    return targetNodeParent
  while child.has_child(flipDir):
    child = child.get_child(flipDir)
